...
def move():
    ...
    #collision detection
    #x<0(left edge)+WINDOW_WIDTH-TILE_SIZE(right edge)
    if snake.x<0:
        snake.x=WINDOW_WIDTH-TILE_SIZE
    #x>=WINDOW_WIDTH(right edge)+0(left edge)
    elif snake.x>=WINDOW_WIDTH:
        snake.x=0
    #y<0(top edge)+WINDOW_HEIGHT(bottom edge)
    if snake.y<0:
        snake.y=WINDOW_HEIGHT-TILE_SIZE
    #y>=WINDOW_HEIGHT(bottom edge)+0(top edge)
